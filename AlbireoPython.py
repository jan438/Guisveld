...
def mizar(c, x, y):
    renderPDF.draw(scaleSVG("SVG/mizar.svg", 0.8), c, x, y + 12.5)
...
def albireo(c, x, y):
    ...
    c.radialGradient(middle_x, speaker_y, speaker_r + 25, ("#EEEFEA", "#e9eae5"), extend=False)
    ...
    c.drawString(middle_x - 0.5 * strwidth, speaker_y + 8.0, "ALβIREO")
    c.setFont(cheatsheetfont, 18)
    strwidth = pdfmetrics.stringWidth("GEN3", cheatsheetfont, 18)
    c.drawString(middle_x - 0.5 * strwidth, speaker_y - 13.0, "GEN3")
    renderPDF.draw(scaleSVG("SVG/star.svg", 0.15), c, middle_x - 12.0, speaker_y + 12.5)
    ...
    c.radialGradient(middle_x, alarmbutton_y, alarmbutton_r + 15, ("#EEEFEA", "#e9eae5"), extend=False)
