...
def albireo(c, x, y):
    ...
    c.drawString(middle_x, speaker_y, "Albireo")
    c.drawString(middle_x, speaker_y, "gen3")
    ...
    c.setFillColor(HexColor('#7eff9c'))
    c.rect(beziers_x + beziers_width - 60, resetbutton_y - 10, 18, 18, stroke=0, fill=1)
    c.setFillColor(HexColor('#ffffff'))
    p = c.beginPath()
    radius = 12
    p.moveTo(middle_x-radius, y+73)
    p.lineTo(middle_x+radius, y+73)
